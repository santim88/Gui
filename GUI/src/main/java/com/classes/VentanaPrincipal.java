...
public class VentanaPrincipal extends JFrame implements ActionListener {
    ...
    private void inicio() {
        ...
        anadir.setBounds(115, 150, 80, 23);
    ...
    private void anadirPersona() {
        ...
        if (existePersona(nombre, apellidos)) {
            JOptionPane.showMessageDialog(null, "La persona con el mismo nombre y apellidos ya está en la lista", "Advertencia", JOptionPane.WARNING_MESSAGE);
            return;
        }

        Persona p = new Persona(nombre, apellidos, campoTelefono.getText(), campoDireccion.getText());
        lista.anadirPersona(p);
        String elemento = nombre + "-" + apellidos + "-" + campoTelefono.getText() + "-" + campoDireccion.getText();
    ...
    private boolean existePersona(String nombre, String apellidos) {
        for (int i = 0; i < lista.getSize(); i++) {
            Persona p = lista.getPersona(i);
    ...
    private void editarPersona(int indice) {
        if (indice >= 0) {
            Persona p = lista.getPersona(indice);
